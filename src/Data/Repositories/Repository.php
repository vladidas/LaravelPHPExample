...
class Repository
{
    ...
    public function count(array $conditions = [])
    {
        ...
        if (array_key_exists('date_start', $conditions)) {
            $query = $query->whereDate('created_at', '>=', $conditions['date_start']);
            unset($conditions['date_start']);
        }

        if (array_key_exists('date_end', $conditions)) {
            $query = $query->whereDate('created_at', '<=', $conditions['date_end']);
    ...
    public function sum(string $field, array $conditions = [])
    {
        ...
        return $query->where($conditions)->sum($field);
    }
    ...
    public function getPaginator(
        ...
        return $this->model->with($relations)->orderBy($orderBy, $sorting)->paginate($perPage);
    }
     ...
     * @return int
     */
    public function increment(int $id, string $field, float $value): int
    {
        return $this->model->find($id)->increment($field, $value);
    }
     ...
     * @return int
     */
    public function decrement(int $id, string $field, float $value): int
    {
        return $this->model->find($id)->decrement($field, $value);
    }
}
